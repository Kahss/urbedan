...
Attribue également un `niveau` (entier de 1 à 3, 3 étant le plus puissant) cohérent avec la puissance relative du personnage face au reste du roster de `data/combattants.json` (les niveaux existants y servent de référence). Rappel : la somme des `niveau` des 4 Combattants d'une équipe ne peut pas dépasser 8, donc éviter de sur-attribuer le niveau 3.
