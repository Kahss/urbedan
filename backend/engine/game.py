...
Version "des par personnage" : plus de deck ni de main de Glyphes. Une fois les deux
Combattants du duel choisis, chaque joueur constitue son pool de des (les des personnels
de son Combattant + les des adverses du Combattant d'en face), lance le tout, et en tire
sa Puissance et son Energie pour ce duel.
"""
import json
import random

from .des import lancer
...
def composer_pool(template_soi, template_adverse):
    """Pool de des lance par `template_soi` : ses propres des personnels, augmentes des
    des adverses inscrits sur la carte du Combattant d'en face."""
    return (
        [(d, "personnel") for d in template_soi.des_personnels]
        + [(d, "adverse") for d in template_adverse.des_adverses]
    )


def lancer_pool(pool):
    """Lance un pool (liste de couples (type_de, origine)) et retourne les resultats,
    chacun annote de son origine (`personnel` ou `adverse`) pour l'affichage."""
    resultats = lancer([type_de for type_de, _ in pool])
    for resultat, (_, origine) in zip(resultats, pool):
...
class Partie:
    ...
    def _choix_ia(self, joueur, role, attr_combattant):
        """L'IA choisit son Combattant via une heuristique simple (cf. ia.py). Si elle
        joue en second, elle connait deja le Combattant adverse (donc les des exacts des
        deux pools) ; si elle joue en premier, elle raisonne sur la moyenne des des
        adverses que peuvent lui donner les Combattants encore disponibles en face."""
        ...
        instance = choisir_combattant(
            joueur, adversaire, combattant_adverse, role,
    ...
    def _resoudre_duel_courant(self):
        ...
        jet_j1 = lancer_pool(composer_pool(template_j1, template_j2))
        jet_j2 = lancer_pool(composer_pool(template_j2, template_j1))

        resultat = resoudre_duel(
            self.j1, self.combattant_j1, jet_j1,
            self.j2, self.combattant_j2, jet_j2,
        ...
        resultat["combattant_j1"] = self._info_cote(template_j1, jet_j1, "humain" if joueur_humain_est_j1 else "ia")
        resultat["combattant_j2"] = self._info_cote(template_j2, jet_j2, "ia" if joueur_humain_est_j1 else "humain")
