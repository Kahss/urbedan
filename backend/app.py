...
# (champs manquants) plutot que d'echouer au premier appel API.
...
@app.get("/api/des")
def api_des():
    """Catalogue statique des 3 types de des (faces, moyennes), pour la legende du
    frontend."""
...
def api_draft():
    ...
    body = request.get_json(force=True) or {}
    try:
        etat = partie.drafter_de(body.get("de_id"))
    except ErreurPartie as e:
        return jsonify({"erreur": str(e)}), 400
    return jsonify(etat)
